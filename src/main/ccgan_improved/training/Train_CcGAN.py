...
def train_CcGAN(
    ...
    if save_models_folder is not None and resume_niters > 0:
        save_file = (
            save_models_folder
            + "/CcGAN_{}_checkpoint_intrain/CcGAN_checkpoint_niters_{}.pth".format(
    ...
    for niter in range(resume_niters, niters):
        ...
        if clip_label:
            ...
            while len(indx_clip) > 1:
                ...
                batch_target_labels_with_epsilon[indx_clip] = (
                    batch_target_labels_in_dataset[indx_clip] + batch_epsilons
                )
                indx_clip = find_outliers(batch_target_labels_with_epsilon)
        ...
        d_loss = torch.mean(real_weights.view(-1) * d_loss_real.view(-1)) + torch.mean(
...
def SampCcGAN_given_label(netG, net_y2h, label, path=None, NFAKE=10000, batch_size=500):
    ...
    if batch_size > NFAKE:
        batch_size = NFAKE
    fake_images = np.zeros(
        (NFAKE + batch_size, NC, IMG_SIZE, IMG_SIZE), dtype=np.float64
    ...
    with torch.no_grad():
        ...
        while tmp < NFAKE:
            ...
            fake_images[tmp : (tmp + batch_size)] = (
                batch_fake_images.cpu().detach().numpy()
            )
            tmp += batch_size
    ...
    if path is not None:
        ...
        print(
            f"Sampling Info: Array shape {raw_fake_images.shape}, Max value {raw_fake_images.max()}"
        )
