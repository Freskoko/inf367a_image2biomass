...
def train_net_embed(
    ...
    if save_models_folder is not None and resumeepoch > 0:
        save_file = (
            save_models_folder
            + "/embed_cnn_checkpoint_intrain/embed_cnn_checkpoint_epoch"
            + str(resumeepoch)
            + ".pth"
        )
    ...
    for epoch in range(resumeepoch, epochs):
        ...
        if testloader is None:
            ...
            with torch.no_grad():
                ...
                for batch_test_images, batch_test_labels in testloader:
                    ...
                    batch_test_labels = (
                        batch_test_labels.type(torch.float).view(-1, 1).to(device)
                    )
        ...
        if save_models_folder is not None and (
            ...
            save_file = (
                save_file + "/embed_cnn_checkpoint_epoch" + str(epoch + 1) + ".pth"
            )
            torch.save(
                {
                    "epoch": epoch,
