...
def train_cGAN(images, labels, netG, netD, save_images_folder, save_models_folder=None):
    ...
    unique_labels = np.sort(np.array(list(set(labels)))).astype(
        int
    ...
    if save_models_folder is not None and resume_niters > 0:
        save_file = (
            save_models_folder
            + "/cGAN_checkpoint_intrain/cGAN_checkpoint_niters_{}.pth".format(
                resume_niters
            )
        )
    ...
    for niter in range(resume_niters, niters):
        ...
        if (niter + 1) % 20 == 0:
            print(
                "cGAN: [Iter %d/%d] [D loss: %.4f] [G loss: %.4f] [D out real:%.4f] [D out fake:%.4f] [Time: %.4f]"
                % (
                    niter + 1,
                    niters,
                    d_loss.item(),
...
def SampcGAN_given_label(
    netG, given_label, class_cutoff_points, NFAKE=10000, batch_size=500
):
    """
    ...
    with torch.no_grad():
        ...
        while tmp < NFAKE:
            z = torch.randn(batch_size, dim_gan, dtype=torch.float).to(device)
            labels = (
                torch.from_numpy(given_class_label * np.ones(batch_size))
                .type(torch.long)
                .to(device)
            )
            batch_fake_images = netG(z, labels)
            fake_images[tmp : (tmp + batch_size)] = (
                batch_fake_images.detach().cpu().numpy()
            )
            tmp += batch_size
