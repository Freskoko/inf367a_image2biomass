...
parser.add_argument(
    "--base_lr", type=float, default=1e-3, help="learning rate, default=1e-3"
)
parser.add_argument(
    "--lr_decay_epochs", type=int, default=50
)  # decay lr rate every dre_lr_decay_epochs epochs
parser.add_argument("--lr_decay_factor", type=float, default=0.1)
parser.add_argument(
    "--lambda_sparsity", type=float, default=0, help="penalty for sparsity"
)
parser.add_argument(
    "--weight_dacay", type=float, default=1e-5, help="Weigth decay, default=1e-5"
)
parser.add_argument(
    "--seed", type=int, default=2020, metavar="S", help="random seed (default: 1)"
)
parser.add_argument("--CVMode", action="store_true", default=False, help="CV mode?")
parser.add_argument(
    "--transform",
    action="store_true",
    default=False,
...
save_AE_images_in_train_folder = (
    wd + "/output/saved_images/AE_lambda_{}_images_in_train".format(lambda_sparsity)
)
os.makedirs(save_AE_images_in_train_folder, exist_ok=True)
save_AE_images_in_valid_folder = (
    wd + "/output/saved_images/AE_lambda_{}_images_in_valid".format(lambda_sparsity)
)
...
if args.CVMode:
    ...
    trainloader = torch.utils.data.DataLoader(
        trainset, batch_size=args.batch_size_train, shuffle=True
    )
    validset = IMGs_dataset(images[indx_valid], labels=None, normalize=True)
    validloader = torch.utils.data.DataLoader(
        validset, batch_size=args.batch_size_valid, shuffle=False
    )
...
def train_AE():
    ...
    if resume_epoch > 0:
        ...
        ckpt_fullpath = (
            save_models_folder
            + "/AE_checkpoint_intrain/AE_checkpoint_epoch_{}_lambda_{}.pth".format(
    ...
    for epoch in range(resume_epoch, epochs):
        ...
        for batch_idx, batch_real_images in enumerate(trainloader):
            ...
            """
            loss = (
                criterion(batch_recons_images, batch_real_images)
                + lambda_sparsity * batch_features.mean()
            )
            ...
            if gen_iterations % 100 == 0:
                ...
                save_image(
                    batch_recons_images.data,
                    save_AE_images_in_train_folder + "/{}.png".format(gen_iterations),
                    nrow=n_row,
                    normalize=True,
                )
            ...
            if gen_iterations % 20 == 0:
                print(
                    "AE+lambda{}: [step {}] [epoch {}/{}] [train loss {}] [Time {}]".format(
                        lambda_sparsity,
                        gen_iterations,
                        epoch + 1,
                        epochs,
    ...
    def valid_AE():
        ...
        with torch.no_grad():
            for batch_idx, images in enumerate(validloader):
                ...
                save_image(
                    recons_images.data,
                    save_AE_images_in_valid_folder + "/{}_recons.png".format(batch_idx),
                    nrow=10,
                    normalize=True,
                )
                save_image(
                    images.data,
                    save_AE_images_in_valid_folder + "/{}_real.png".format(batch_idx),
                    nrow=10,
                    normalize=True,
                )
        return None
...
filename_ckpt = save_models_folder + "/ckpt_AE_epoch_{}_seed_{}_CVMode_{}.pth".format(
    args.epochs, args.seed, args.CVMode
)
