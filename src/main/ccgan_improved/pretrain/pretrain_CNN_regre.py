...
parser.add_argument(
    "--CNN", type=str, default="ResNet34_regre", help="CNN for training; ResNetXX"
)
parser.add_argument(
    "--epochs",
    type=int,
...
parser.add_argument(
    "--base_lr", type=float, default=0.01, help="learning rate, default=0.1"
)
parser.add_argument(
    "--weight_dacay", type=float, default=1e-4, help="Weigth decay, default=1e-4"
)
parser.add_argument(
...
def valid_CNN(verbose=True):
    ...
    with torch.no_grad():
        ...
        if verbose:
            print(
                "Validation Average Absolute Difference: {}".format(
                    abs_diff_avg / total
                )
            )
...
images_subset = np.zeros(
    (n_unique_cellcount * 1000, 3, args.img_size, args.img_size), dtype=np.uint8
)
...
for i in range(n_unique_cellcount):
    ...
    if i == 0:
        ...
        images_subset = np.concatenate(
            (images_subset, images_train[index_curr_cellcount]), axis=0
        )
...
optimizer = torch.optim.SGD(
    net.parameters(), lr=args.base_lr, momentum=0.9, weight_decay=args.weight_dacay
)
