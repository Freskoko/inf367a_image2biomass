...
class decoder(nn.Module):
    def __init__(self, dim_bottleneck=512, ch=64):
        ...
        self.deconv = nn.Sequential(
            nn.ConvTranspose2d(
                ch * 8, ch * 8, kernel_size=4, stride=2, padding=1
            ),  # h=2h; 8
            nn.BatchNorm2d(ch * 8),
            nn.ReLU(True),
            nn.Conv2d(ch * 8, ch * 8, kernel_size=3, stride=1, padding=1),  # h=h
            ...
            nn.ReLU(),
            nn.ConvTranspose2d(
                ch * 8, ch * 4, kernel_size=4, stride=2, padding=1
            ),  # h=2h; 16
            nn.BatchNorm2d(ch * 4),
            nn.ReLU(True),
            nn.Conv2d(ch * 4, ch * 4, kernel_size=3, stride=1, padding=1),  # h=h
            ...
            nn.ReLU(),
            nn.ConvTranspose2d(
                ch * 4, ch * 2, kernel_size=4, stride=2, padding=1
            ),  # h=2h; 32
            nn.BatchNorm2d(ch * 2),
            nn.ReLU(True),
            nn.Conv2d(ch * 2, ch * 2, kernel_size=3, stride=1, padding=1),  # h=h
            ...
            nn.ReLU(),
            nn.ConvTranspose2d(
                ch * 2, ch, kernel_size=4, stride=2, padding=1
            ),  # h=2h; 64
            nn.BatchNorm2d(ch),
            nn.ReLU(True),
            nn.Conv2d(ch, ch, kernel_size=3, stride=1, padding=1),  # h=h
